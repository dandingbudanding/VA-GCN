...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='VAGCN_part_seg', help='model name')
    ...
    parser.add_argument('--log_dir', type=str, default='VAGCN_part_seg1', help='log path')
...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 250, eta_min=0.00001, last_epoch=-1)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            label_one_hot = torch.from_numpy(label_one_hot.astype(np.float32)).cuda()
            ...
            seg_pred = classifier(points, label_one_hot)
