...
class EAConv(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, norm='bn'):
        ...
        #self.lins = nn.ModuleList([nn.Linear(in_channels, hidden_channels) for _ in range(1)])
        self.norm1 = Norm(norm, in_channels, md=1)
    ...
    def forward(self, x, p, B, n, id_euc):
        ...
        #edge = self.lins(edge).reshape(1, B, n, k, -1)  # [bases, B*n*k, C]
        edge = edge.reshape(1, B, n, k, -1)
...
class get_model(nn.Module):
    def __init__(self, num_classes, norm='bn'):
        ...
        self.conv1 = EAConv(16, 16, 16, norm=norm)
        self.lin2 = MLP_md([32, 32])
        self.conv2 = EAConv(32, 32, 32, norm=norm)
        self.res_conv = MLP_md([32, 64])

        # master
        self.conv3 = EAConv(256, 64, 256, norm=norm)
        self.lin2_1 = MLP_md([512, 1024], norm=norm)
        self.lin2_2 = MLP_md([16, 64], norm=norm)

        self.lin3 = MLP_md([1088 + 128 * 2 + 256, 512], norm=norm)
        self.dp1 = nn.Dropout(p=0.5)
        self.lin4 = MLP_md([512, 256], norm=norm)
        self.dp2 = nn.Dropout(p=0.5)
        self.lin5 = MLP_md([256, 128], norm=norm)
        self.lin6 = MLP_md([128, num_classes], norm=norm)

    def forward(self, xyz, l):
        ...
        x5 = torch.cat([x5, x5_2], dim=1)  # [B*N, C] 64
        ...
        x7 = torch.cat([x7, x7_2], dim=1)  # [B*N, C] 256

        x6 = self.res_conv(x6)  # 256

        # master
        x8 = torch.cat([x2, x6, x7], dim=-1)  # [B*N, 960]
        x8_2 = self.conv3(x8, pos, B, N, id_euc)  # [B*N, C]
        id_euc = sphg(pos, 0.8, B, max_num_neighbors=16)
        x8_3 = self.conv3(x8, pos, B, N, id_euc)  # [B*N, C]
        x8_2 = torch.cat([x8_2, x8_3], dim=1)  # [B*N, C]

        x9 = self.lin2_1(x8_2.reshape(B, N, -1))
        x9 = torch.max(x9, dim=1, keepdim=True)[0]  # [B, 1, 1024]

        l = l.view(B, -1, 1).permute(0, 2, 1)  # [B, 1, 16]
        x9_1 = self.lin2_2(l)  # [B, 1, 64]
        x9 = torch.cat([x9, x9_1], dim=-1)  # [B, 1, 1088]

        x9 = x9.repeat(1, N, 1).flatten(0, 1)  # [B*N, 1088]
        x9 = torch.cat([x9, x8, x8_3], dim=1)  # [B*N, 1088+960]

        x10 = self.lin3(x9.view(B, N, -1)).view(B * N, -1)
        x10 = self.dp1(x10)
        x10 = self.lin4(x10.view(B, N, -1)).view(B * N, -1)
        x10 = self.dp2(x10)
        x10 = self.lin5(x10.view(B, N, -1)).view(B * N, -1)
        x = self.lin6(x10.view(B, N, -1))
